...
export default function DataCenter() {
  ...
  const [typeTier, setTypeTier] = useState<"L" | "M" | "H">("M");
  ...
  const handleRunEvaluation = () => ingestMutation.mutate({ machineId, type: typeTier, airTempK: values.airTempK, processTempK: values.processTempK, rpm: values.rpm, torqueNm: values.torqueNm, toolWearMin: values.toolWearMin, vibrationRmsMmS: values.vibrationMmS });
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="bg-slate-900/90 border-slate-800">
        ...
        <CardContent className="p-4 sm:p-5 space-y-5"><div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">{telemetryFields.map((field) => <div key={field.key}><label className="text-[11px] font-mono text-slate-400 block mb-1">{field.label}</label><Input type="number" step={field.step} value={values[field.key]} onChange={(event) => updateValue(field.key, parseFloat(event.target.value) || 0)} className="bg-slate-950 border-slate-800 text-xs font-mono text-white" /></div>)}<div><label className="text-[11px] font-mono text-slate-400 block mb-1">AI4I Type</label><select value={typeTier} onChange={(event) => setTypeTier(event.target.value as "L" | "M" | "H")} className="h-9 w-full rounded-md border border-slate-800 bg-slate-950 px-3 text-xs font-mono text-white"><option value="L">L · Low</option><option value="M">M · Medium</option><option value="H">H · High</option></select></div></div><div className="flex justify-end"><Button onClick={handleRunEvaluation} className="bg-amber-500 hover:bg-amber-400 text-slate-950 font-semibold text-xs"><Sparkles className="w-3.5 h-3.5 mr-1.5" />Ingest & Evaluate Compatible Record</Button></div>{evaluatedResult && <div className="p-4 rounded bg-slate-950 border border-slate-800 space-y-3"><div className="flex items-center justify-between border-b border-slate-800 pb-2"><span className="text-xs font-bold text-white font-mono">Rule-engine output</span><RiskPill category={evaluatedResult.assessed.riskCategory} pct={evaluatedResult.assessed.estimatedRiskPct} /></div><div className="text-xs text-slate-200"><strong>Explanation:</strong> {evaluatedResult.assessed.explanation}</div><div className="text-xs text-amber-300"><strong>Recommendation:</strong> {evaluatedResult.assessed.recommendedAction}</div><div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-[11px] font-mono text-slate-400"><div>Power: <strong className="text-white">{evaluatedResult.normalizedTelemetry.powerKw} kW</strong></div><div>Temp ΔT: <strong className="text-white">{evaluatedResult.normalizedTelemetry.tempDiffC}°C</strong></div><div>Anomaly: <strong className="text-white">{evaluatedResult.assessed.anomalyScore}</strong></div><div>Health: <strong className="text-white">{evaluatedResult.assessed.healthScorePct}/100</strong></div></div><div className="p-3 rounded border border-sky-500/30 bg-sky-500/5 text-xs text-sky-200"><div className="font-mono font-bold">AI4I model prediction</div><div className="mt-1">{evaluatedResult.modelPrediction?.label}: <strong>{evaluatedResult.modelPrediction?.probabilityPct ?? "—"}%</strong></div><div className="text-sky-300/70 mt-1">Model output from uploaded AI4I features; rule-engine risk above remains a separate transparent assessment.</div></div></div>}</CardContent>
      </Card>
